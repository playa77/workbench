...
(function () {
  ...
  function buildEquation() {
    ...
    if (!eqType || (!lhs)) {
      alert('Select an equation type and fill in at minimum the left-hand side.');
      return;
    }
  ...
  async function startSession() {
    ...
    if (!problemText) { alert('Describe your problem first.'); return; }
  ...
  function advanceInterviewStep() {
    ...
    if (step.id === 'lhs') {
      var val = step.getValue();
      if (!val) { alert('The left-hand side is required.'); return; }
      interviewData.lhs = val;
    } else if (step.id === 'type') {
      var val = step.getValue();
      if (!val) { alert('Please select an equation type.'); return; }
  ...
  async function finishInterview() {
    ...
    if (!window._iwEquationJson || !window._iwEquationLatex) return;
