...
(function () {
  ...
  function renderSetupPhase() {
    return ''
      ...
      +     '<textarea class="mt-problem-input" id="mt-problem-text" placeholder="Describe your math problem in your own words. Include any equations (use $$ for LaTeX) — for example:\n\nSolve: $$\\frac{d^2x}{dt^2} + 3\\frac{dx}{dt} + 2x = 0$$\nwith initial conditions $$x(0) = 1, \\dot{x}(0) = 0$$\n\nOr describe a word problem: A tank contains 100 L of solution with 10 kg of salt..." data-tooltip="Enter a math problem in natural language or LaTeX notation. Examples: \'Solve x^2 - 5x + 6 = 0\' or \'Find the derivative of sin(x^2)\'" data-help-page="/static/help/math-tutor.html#problem"></textarea>'
      +   '</div>'
      +   '<div style="font-size:12px;color:var(--text-muted);margin-bottom:12px">'
      +     '<a href="#" id="mt-show-builder" style="color:var(--accent);text-decoration:none" data-tooltip="Show the visual equation builder to construct mathematical expressions in LaTeX." data-help-page="/static/help/math-tutor.html#equation-builder">^ Equation Builder (structured equation input)</a>'
      +   '</div>'
      ...
      +     '<button class="btn btn-primary" id="btn-mt-start" data-tooltip="Begin a tutoring session. The agent initializes with your problem and guides you step-by-step." data-help-page="/static/help/math-tutor.html#start-session">Start Tutor Session</button>'
      +     '<button class="btn btn-secondary" id="btn-mt-build" style="display:none" data-tooltip="Build a LaTeX equation from the selected type, order, and coefficients." data-help-page="/static/help/math-tutor.html#equation-builder">Build Equation</button>'
      +   '</div>'
      + '</div>';
  }
  ...
  function renderEquationBuilder() {
    return ''
      ...
      +   '<div class="mt-eq-row">'
      +     '<select id="mt-eq-type" style="min-width:140px" data-tooltip="Select the type of equation to build: polynomial, trigonometric, exponential, etc." data-help-page="/static/help/math-tutor.html#equation-builder">'
      ...
      +       '<option value="optimization">Optimization</option>'
      +     '</select>'
      +     '<input type="number" id="mt-eq-order" placeholder="Order (e.g. 2)" style="width:90px" min="0" data-tooltip="Set the order/degree of the equation (e.g., 2 for quadratic)." data-help-page="/static/help/math-tutor.html#equation-builder" />'
  ...
  function renderChatPhase(startBody) {
    ...
    chatArea.innerHTML = ''
      ...
      +       '<button class="btn btn-secondary btn-sm" id="btn-mt-build-eq" data-tooltip="Build a LaTeX equation to include in your chat message." data-help-page="/static/help/math-tutor.html#build-equation-chat">+ Build Equation</button>'
      +       '<button class="btn btn-secondary btn-sm" id="btn-mt-interview" data-tooltip="Start a structured multiple-choice assessment with conceptual questions about the current topic." data-help-page="/static/help/math-tutor.html#mc-interview">Comprehensive MC Interview</button>'
      +       '<button class="btn btn-danger btn-sm" id="btn-mt-end" style="margin-top:4px" data-tooltip="End the current tutoring session. The full transcript is saved to your session history." data-help-page="/static/help/math-tutor.html#end-session">End Session</button>'
      +     '</div>'
      +   '</div>'
      ...
      +       '<input class="form-input" id="mt-chat-input" placeholder="Ask a question or discuss a step..." onkeydown="if(event.key===\'Enter\')window._mtSendMessage()" data-tooltip="Ask a follow-up question during the tutoring session. The tutor responds with LaTeX-rendered explanations." data-help-page="/static/help/math-tutor.html#chat" />'
      +       '<button class="btn btn-primary" id="btn-mt-send" data-tooltip="Send your chat message to the math tutor during an active session." data-help-page="/static/help/math-tutor.html#chat">Send</button>'
      +     '</div>'
      +   '</div>'
      + '</div>';
  ...
  function renderMcq(mcq) {
    ...
    area.innerHTML = ''
      ...
      +   (Object.keys(mcq.options || {}).map(function (key) {
          return '<div class="mt-mcq-option" data-option="' + Utils.escapeHtml(key) + '" onclick="window._mtSelectOption(this)" data-tooltip="Select this answer for the multiple-choice question." data-help-page="/static/help/math-tutor.html#mc-interview">'
            + '<div class="mt-mcq-radio"></div>'
            + '<span>' + Utils.escapeHtml(key) + '. ' + Utils.escapeHtml(mcq.options[key]) + '</span>'
            + '</div>';
        }).join(''))
      +   '<div style="margin-top:12px;display:flex;gap:8px;align-items:center">'
      +     '<button class="btn btn-primary btn-sm" id="btn-mt-submit-answer" onclick="window._mtSubmitAnswer()" data-tooltip="Submit your answer for the current multiple-choice question." data-help-page="/static/help/math-tutor.html#mc-interview">Submit Answer</button>'
  ...
  function renderInterviewOverlay() {
    ...
    overlay.innerHTML = ''
      ...
      +     '<button class="btn btn-secondary btn-sm" id="btn-iw-back" style="display:none" data-tooltip="Return to the previous interview question." data-help-page="/static/help/math-tutor.html#mc-interview">^ Back</button>'
      +     '<div style="flex:1"></div>'
      +     '<button class="btn btn-secondary btn-sm" id="btn-iw-skip" style="display:none" data-tooltip="Skip the current question and move to the next." data-help-page="/static/help/math-tutor.html#mc-interview">Skip</button>'
      +     '<button class="btn btn-primary btn-sm" id="btn-iw-next" data-tooltip="Move to the next interview question." data-help-page="/static/help/math-tutor.html#mc-interview">Next v</button>'
      +     '<button class="btn btn-primary btn-sm" id="btn-iw-finish" style="display:none" data-tooltip="Complete the interview and view results." data-help-page="/static/help/math-tutor.html#mc-interview">Send to Tutor</button>'
      +   '</div>'
      + '</div>';
