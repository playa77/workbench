...
(function () {
  ...
  async function createKb() {
    ...
    if (!name) return;
    var btn = document.getElementById("kb-new-create");
  ...
  function renderDocumentsList() {
    ...
    el.querySelectorAll(".kb-doc-delete").forEach(function (btn) {
      btn.addEventListener("click", async function (e) {
        ...
        await deleteDocument(btn, docId);
      });
    });
  }

  async function deleteDocument(btn, docId) {
  ...
  async function kbQuerySend() {
    var sendBtn = document.querySelector('#kb-right-panel .btn-primary');
    var input = document.getElementById("kb-query-input");
    var question = input.value.trim();
    if (!question) return;
    input.value = "";
    Utils.setButtonLoading(sendBtn);
