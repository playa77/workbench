"""Debate Plugin — multi-agent debate system.
...
class DebatePlugin(PluginBase):
    ...
    async def start_debate(self, body: "DebateRequest", user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
