"""Deliberation Plugin — multi-frame deliberation engine.
...
class DeliberationPlugin(PluginBase):
    name = "deliberation"
    ...
    async def analyze(self, body: "DeliberationRequest", user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
