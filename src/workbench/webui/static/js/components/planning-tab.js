...
(function () {
  ...
  function renderPlanningTab(container) {
    ...
    container.innerHTML = ''
      ...
      +       '<textarea class="form-input" id="pl-goal" placeholder="Describe your goal, project, or problem..." style="min-height:80px" data-tooltip="Describe what you want to achieve. Include scope, timeline, resources, and constraints for a better plan." data-help-page="/static/help/planning.html#goal"></textarea>'
      +     '</div>'
      +     '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">'
      +       '<div class="form-group">'
      +         '<label>Plan Type</label>'
      +         '<select class="form-input" id="pl-type" data-tooltip="Choose from 9 specialized plan types: Project Plan, Business Strategy, Technical Architecture, Marketing Campaign, Product Roadmap, Risk Mitigation, Learning Path, Operational Workflow, or Financial Model." data-help-page="/static/help/planning.html#plan-type"><option value="">Loading types...</option></select>'
      +       '</div>'
      +       '<div class="form-group">'
      +         '<label>Model</label>'
      +         '<select class="form-input" id="pl-model" disabled data-tooltip="Choose which LLM to use. The strong model from your default provider is pre-selected — planning benefits from capable models." data-help-page="/static/help/planning.html#model"><option>Loading models...</option></select>'
      +       '</div>'
      +       '<div class="form-group">'
      +         '<label>Temperature</label>'
      +         '<input class="form-input" id="pl-temperature" type="number" value="0.5" min="0" max="2" step="0.1" data-tooltip="Creativity level (0.0–2.0). Lower values = more structured, higher = more innovative approaches. Default: 0.7." data-help-page="/static/help/planning.html#temperature" />'
      +       '</div>'
      +     '</div>'
      +     '<div style="display:flex;gap:8px">'
      +       '<button class="btn btn-primary" id="btn-start-plan" data-tooltip="Generate the plan. Progress streams via SSE with live updates." data-help-page="/static/help/planning.html#generate">Generate Plan</button>'
      +       '<button class="btn btn-danger" id="btn-stop-plan" data-tooltip="Abort a running plan generation. Partial results are preserved." data-help-page="/static/help/planning.html#generate" style="display:none">Stop</button>'
      +     '</div>'
      +   '</div>'
  ...
  function renderPlanResult(content) {
    ...
    output.innerHTML = ''
      ...
      +     '<button class="btn btn-primary btn-sm" onclick="window.planningCopyResult()" data-tooltip="Copy the plan to your clipboard." data-help-page="/static/help/planning.html#export">Copy</button>'
      +     '<span id="planning-template-picker" data-tooltip="Choose a PDF template for export." data-help-page="/static/help/planning.html#template-selector"></span>'
      +     '<button class="btn btn-secondary btn-sm" onclick="window.planningExportHtml()" data-tooltip="Download the plan as a formatted HTML file." data-help-page="/static/help/planning.html#export">Export HTML</button>'
      +     '<button class="btn btn-secondary btn-sm" onclick="window.planningExportPdf()" data-tooltip="Download the plan as a PDF using the selected template." data-help-page="/static/help/planning.html#export">Export PDF</button>'
      +     '<button class="btn btn-secondary btn-sm" onclick="Router.setActive(\'planning\')" data-tooltip="Clear current results and return to the input form." data-help-page="/static/help/planning.html#export">New Plan</button>'
      +   '</div>'
      + '</div>';
  ...
  function renderPlanningPastSessions() {
    ...
    output.insertAdjacentHTML('afterend', ''
      ...
      +   '<div class="card-header" style="cursor:pointer;display:flex;justify-content:space-between;align-items:center" id="planning-past-toggle" data-tooltip="Click to show or hide previous planning sessions. Click to reload a plan." data-help-page="/static/help/planning.html#past-plans">'
