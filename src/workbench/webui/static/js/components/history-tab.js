...
(function () {
  ...
  var _historyState = { mode: 'list' };

  Router.onActivate('history', function (prevTab) {
    if (_historyState.mode === 'detail') {
      _historyState = { mode: 'list' };
      var panel = Router._getOrCreatePanel('history');
      renderHistoryTab(panel);
    }
  });

  window.addEventListener('popstate', function (e) {
    if (e.state && e.state._historyMode === 'detail') {
      _historyState = { mode: 'detail', sessionId: e.state.sessionId };
      popstateViewSession(e.state.sessionId);
    } else {
      _historyState = { mode: 'list' };
      var panel = document.getElementById('tab-panel-history');
      if (panel) {
        renderHistoryTab(panel);
      }
    }
  ...
  function renderHistoryTab(container) {
    container.innerHTML = ''
      ...
      +     '</select>'
      +     '<label style="font-size:13px;color:var(--text-secondary);font-weight:500;margin-left:4px">PDF Style:</label>'
    ...
    API.listSessions(selectedAgent || undefined)
      .then(function (sessions) {
        ...
        content.querySelectorAll('[data-action="pdf"]').forEach(function (btn) {
          btn.addEventListener('click', function () { exportSessionPdf(btn, btn.dataset.id, btn.dataset.title); });
        });
        content.querySelectorAll('[data-action="delete"]').forEach(function (btn) {
          btn.addEventListener('click', function () { deleteSession(btn, btn.dataset.id, btn.dataset.title); });
        });
      })
      .catch(function () {
  ...
  function goBackToList() {
    _historyState = { mode: 'list' };
    history.pushState({ _historyMode: 'list' }, '');
    var panel = Router._getOrCreatePanel('history');
    renderHistoryTab(panel);
  }

  function viewSession(id) {
    _historyState = { mode: 'detail', sessionId: id };
    history.pushState({ _historyMode: 'detail', sessionId: id }, '');
  ...
  function popstateViewSession(id) {
    var content = document.getElementById('history-content');
    if (!content) {
      goBackToList();
      return;
  ...
  function exportSessionPdf(btn, id, title) {
    Utils.setButtonLoading(btn, 'Generating PDF...');
    var template = window.getSelectedTemplate ? window.getSelectedTemplate() : 'professional';
    API.getSession(id)
      .then(function (session) {
        return Utils.exportMarkdownAsPdf(session.content || '', title || session.title || 'Session', template, btn);
      })
      .then(function () {
