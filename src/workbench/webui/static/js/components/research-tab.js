...
(function () {
  ...
  function renderResearchTab(container) {
    ...
    container.innerHTML = ''
      ...
      +       '<textarea class="form-input" id="rq-question" placeholder="Enter a research question..." style="min-height:80px" data-tooltip="Enter a clear, well-scoped research question. Example: \'What are the environmental impacts of lithium mining?\'" data-help-page="/static/help/research.html#research-question"></textarea>'
      +     '</div>'
      +     '<div class="form-group">'
      +       '<label>Report Title (optional)</label>'
      +       '<input class="form-input" id="rq-report-title" type="text" placeholder="Leave empty to use the question as title" maxlength="200" data-tooltip="Optional custom title for the generated report. If left empty, the research question is used." data-help-page="/static/help/research.html#report-title" />'
      +     '</div>'
      +     '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">'
      +       '<div class="form-group">'
      +         '<label>Tree Depth (levels, 1–5)</label>'
      +         '<input class="form-input" id="rq-depth" type="number" value="2" min="1" max="5" data-tooltip="Research tree depth (1\u20135). Higher values explore sub-topics more deeply but take exponentially longer. Default: 2." data-help-page="/static/help/research.html#tree-depth" />'
      +       '</div>'
      +       '<div class="form-group">'
      +         '<label>Branching Factor (per node, 1–10)</label>'
      +         '<input class="form-input" id="rq-branching" type="number" value="5" min="1" max="10" data-tooltip="Number of sub-questions per node (1\u201310). Higher values increase breadth. Watch the leaf count estimate below." data-help-page="/static/help/research.html#branching-factor" />'
      +       '</div>'
      +     '</div>'
      ...
      +       '<select class="form-input" id="rq-language" data-tooltip="Output language for the research report. Auto detects from your question if not specified." data-help-page="/static/help/research.html#language">'
      ...
      +       '<button class="btn btn-primary" id="btn-start-research" data-tooltip="Start the deep research process. Progress streams in real-time with live tree visualization." data-help-page="/static/help/research.html#start-research">Start Research</button>'
      +       '<button class="btn btn-danger" id="btn-stop-research" style="display:none" data-tooltip="Abort the running research session. Results collected so far are preserved." data-help-page="/static/help/research.html#stop">Stop</button>'
      +     '</div>'
      +   '</div>'
  ...
  function renderReport(content, title) {
    ...
    output.innerHTML = ''
      ...
      +     '<button class="btn btn-primary btn-sm" onclick="window.researchCopyReport()" data-tooltip="Copy the full research report to your clipboard as Markdown text." data-help-page="/static/help/research.html#export">Copy Markdown</button>'
      +     '<span id="research-template-picker" data-tooltip="Choose a PDF template for export. Templates control styling, fonts, and layout." data-help-page="/static/help/research.html#template-selector"></span>'
      +     '<button class="btn btn-primary btn-sm" id="btn-export-pdf" onclick="window.researchExportPdf()" data-tooltip="Generate and download a PDF version of the research report using the selected template." data-help-page="/static/help/research.html#export">Export PDF</button>'
      +     '<button class="btn btn-secondary btn-sm" onclick="Router.setActive(\'research\')" data-tooltip="Clear the current report and return to the research input form." data-help-page="/static/help/research.html#export">New Research</button>'
      +   '</div>'
      + '</div>';
  ...
  window.researchCopyReport = function () {
    if (window._researchReportContent) {
      navigator.clipboard.writeText(window._researchReportContent)
  ...
  function renderResearchPastSessions() {
    ...
    var pastHtml = ''
      ...
      +   '<div class="card-header" style="cursor:pointer;display:flex;justify-content:space-between;align-items:center" id="research-past-toggle" data-tooltip="Click to expand or collapse the list of previous research sessions. Click a session to load its report." data-help-page="/static/help/research.html#past-research">'
